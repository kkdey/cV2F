...
blood_target_ids = unique(unlist(sapply(c("K562", "CD", "GM", "blood", "Blood", "Lymph", "lymph"),
...
GM12878_target_ids = as.numeric(unique(unlist(sapply(c("GM"), function(x) return(grep(x, target_labels))))))
HepG2_target_ids = as.numeric(unique(unlist(sapply(c("HepG2"), function(x) return(grep(x, target_labels))))))
A549_target_ids = as.numeric(unique(unlist(sapply(c("A549"), function(x) return(grep(x, target_labels))))))
SKNSH_target_ids = as.numeric(unique(unlist(sapply(c("SK"), function(x) return(grep(x, target_labels))))))
...
target_ids_list[["K562"]] = K562_target_ids
target_ids_list[["GM12878"]] = GM12878_target_ids
target_ids_list[["HepG2"]] = HepG2_target_ids
target_ids_list[["A549"]] = A549_target_ids
target_ids_list[["SKNSH"]] = SKNSH_target_ids
...
for(numc in 1:length(celltypes_mode)){
  ...
  colnames(merged_outdf) = c("CHR", "BP", "SNP", "REF", "ALT", "MEANZ", "MAXZ")
...
xx = unique(as.character(sapply(list.files("/n/groups/price/kushal/Imperio/ALL_DEEP/Enformer/Z_SKEW_FEATURES"),
       function(x) return(strsplit(x, "[.]")[[1]][2]))))
write.table(xx, file = "/n/groups/price/kushal/Imperio/ALL_DEEP/Enformer/celltypes_mode.txt",
